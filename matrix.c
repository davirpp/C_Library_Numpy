...
int get_element(struct matrix a_matrix, int ri, int ci){
    ...
    int position = (ri * a_matrix.stride_rows) + (ci * a_matrix.stride_cols) + a_matrix.offset;
...
void print_matrix(struct matrix a_matrix) {
    ...
    for (int i = 0; i < a_matrix.n_rows; i++) {
        ...
        for (int j = 0; j < a_matrix.n_cols; j++) {
            printf(" %*d ", count_numbers(biggest), array[(i * a_matrix.stride_rows) + (j * a_matrix.stride_cols) + a_matrix.offset]);
        }
        printf("|\n");
    }
}
...
struct matrix slice(struct matrix a_matrix, int rs, int re, int cs, int ce) {
    ...
    s_m.offset = (rs * s_m.stride_rows) + (cs * s_m.stride_cols) + a_matrix.offset;
...
int sum(struct matrix a_matrix) {
    ...
    for (int i = 0; i < a_matrix.n_rows; i++) {
        for (int j = 0; j < a_matrix.n_cols; j++) {
            position = (i * a_matrix.stride_rows) + (j  * a_matrix.stride_cols) + a_matrix.offset;
...
int min(struct matrix a_matrix) {
    ...
    int menor = a_matrix.data[a_matrix.offset];
...
int max(struct matrix a_matrix) {
    ...
    int maior = a_matrix.data[a_matrix.offset];
...
struct matrix add(struct matrix a_matrix, struct matrix b_matrix) {
    ...
    if ((a_matrix.n_rows == b_matrix.n_rows) && (a_matrix.n_cols == b_matrix.n_cols)) {
        ...
        for (int i = 0; i < a_matrix.n_rows; i++) {
            for (int j = 0; j < a_matrix.n_cols; j++) {
                position_a = (i * a_matrix.stride_rows) + (j  * a_matrix.stride_cols) + a_matrix.offset;
                position_b = (i * b_matrix.stride_rows) + (j  * b_matrix.stride_cols) + b_matrix.offset;
...
struct matrix matmul(struct matrix a_matrix, struct matrix b_matrix){
    ...
    if (a_matrix.n_cols == b_matrix.n_rows) {
        ...
        int x = 0;
        for (int i = 0; i < a_matrix.n_rows; i++){
            a_slice = slice(a_matrix, i, i+1, 0, a_matrix.n_cols);
            for (int j = 0; j < b_transpose.n_rows; j++){
                b_slice = slice(b_transpose, j, j+1, 0, b_transpose.n_cols);    
